...
compile_src(const std::vector<const char *> &src, int type, std::vector<const char *> const *orig_names)
{
	...
	if(!success) {
		...
		ss << std::endl << infolog << std::endl;

		shader_error_log += ss.str();
...
compile_shader(const ShaderSource &src, const ShaderSource *src_orig)
{
	...
	if(!success) {
		...
		if (src_orig) {
			for (auto x : {
					&src_orig->vertex, &src_orig->fragment,
					&src_orig->geometry, &src_orig->compute,
					&src_orig->tess_control, &src_orig->tess_eval })
			{
				for (auto& i : *x) {
					std::cerr << i << ", ";
				}
...
load_shader(const std::string &path)
{
	...
	auto file_read_with_error = [](const char *file_path) {
		auto x = file_read(file_path);
		if(!x) {
			std::cerr << "error: could not read \"" << file_path << "\"!" << std::endl;
		}
		return x;
	};

	for(auto i: p.files_vertex) {
		if(auto x = file_read_with_error(i)) {
			src.vertex.push_back(x);
			srcOrig.vertex.push_back(i);
		}
	}
	for(auto i: p.files_fragment) {
		if(auto x = file_read_with_error(i)) {
			src.fragment.push_back(x);
			srcOrig.fragment.push_back(i);
		}
	}
	for(auto i: p.files_geom) {
		if(auto x = file_read_with_error(i)) {
			src.geometry.push_back(x);
			srcOrig.geometry.push_back(i);
		}
	}
	for(auto i: p.feedback_varyings) {
		src.feedback_varyings.push_back(i);
	}
	for(auto i: p.files_compute) {
		if(auto x = file_read_with_error(i)) {
			src.compute.push_back(x);
			srcOrig.compute.push_back(i);
		}
	}

	std::string defines = p.defines;
