...
extern std::string shader_error_log;
	...
	uniform(const std::string &str, const T *t, size_t count)
	{
		...
		if(it != uniforms.end()) {
			...
			if(reported_missing_uniforms.find(str) == reported_missing_uniforms.end()) {
				std::cerr << "[" << name << "] Warning: uniform \"" << str << "\" not found!" << std::endl;
...
public:
	...
	void set_program_definitions(const MapNameProgramDefinition &program_definitions_)
	{
		program_definitions = program_definitions_;
	}
...
private:
	...
	MapNameProgramDefinition program_definitions;
};
