...
#include <cglib/imgui/imgui_internal.h>
...
void ImGui::ForceIniSettingsDirty()
{
...
static ImGuiIniData* AddWindowSettings(const char* name)
{
    ...
    ini->Size = ImVec2(0,0);
    ini->FontGlobalScale = 1.f;//ImGui::GetIO().FontGlobalScale;
    return ini;
}
...
static void LoadIniSettingsFromDisk(const char* ini_filename)
{
    ...
    for (const char* line_start = file_data; line_start < buf_end; )
    {
        ...
        else if (settings)
        {
            ...
            else if (sscanf(line_start, "FontGlobalScale=%f", &x) == 1)
                settings->FontGlobalScale = x;
        }
...
static void SaveIniSettingsToDisk(const char* ini_filename)
{
    ...
    for (int i = 0; i != g.Windows.Size; i++)
    {
        ...
        settings->FontGlobalScale = ImGui::GetIO().FontGlobalScale;
    }
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    else
    {
        ...
        {
            ImGui::GetIO().FontGlobalScale = settings->FontGlobalScale;
...
#include <cglib/core/stb_textedit.h>
...
static bool Items_StringVectorGetter(void* data, int idx, const char** out_text)
{
	const std::vector<std::string>* items = (const std::vector<std::string>*)data;
...
{
	const bool value_changed = Combo(label, current_item, Items_StringVectorGetter, (void*)items, items_count, height_in_items);
	return value_changed;
}
